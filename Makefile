REPOSITORY := theovoss/django-chess-api
...
.envrc:
	...
	echo export DATABASE_URL=postgresql://localhost/DjangoChessApi_dev >> $@
...
reset: install ## Database | Create a new database, migrate, and seed it
	- dropdb DjangoChessApi_dev
	- createdb DjangoChessApi_dev
	make data

# VALIDATION TARGETS ##########################################################

PYTHON_PACKAGES := config DjangoChessApi
...
.PHONY: promote
promote: install
	TEST_SITE=https://staging.DjangoChessApi.com $(RUN) pytest tests/system --cache-clear
	heroku pipelines:promote --app DjangoChessApi-staging --to DjangoChessApi
	TEST_SITE=https://DjangoChessApi.com $(RUN) pytest tests/system
